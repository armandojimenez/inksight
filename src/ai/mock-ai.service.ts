...
const IMAGE_ANALYSIS_RESPONSES = [
...
function hashSelect<T>(input: string, array: T[]): T {
  let hash = 5381;
...
function buildCompletion(content: string, promptTokens: number): OpenAiChatCompletion {
...
export class MockAiService implements IAiService {
  ...
  async chat(
    ...
    const content = hashSelect(message, responses);
    const promptTokens = estimateTokens(message) + history.reduce(
  ...
  async *chatStream(
    ...
    const responses =
      history.length > 2 ? FOLLOWUP_CHAT_RESPONSES : DEFAULT_CHAT_RESPONSES;
    const fullResponse = hashSelect(message, responses);
    const id = generateId();
